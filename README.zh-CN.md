...
- `source` 表示**条目的 canonical 收录来源**：例如 GitHub 仓库或 ClawHub skill 页面
- `upstream` 表示**可选的上游仓库**：当 canonical source 是 registry 页面时，用于保留源码出处
...
> 若条目本身是 Skill，其实际 `SKILL.md` 通常位于上游项目仓库或 registry 条目中；本仓库只维护索引。
...
- **Clawhub Skill Creator**: ClawHub 原生的技能创建工具，用于生成并打包可发布到 ClawHub 的 skill
  来源：[clawhub.ai/skills/clawhub-skill-creator](https://clawhub.ai/skills/clawhub-skill-creator)
  类型：`skill`
...
```jsonc
{
  ...
  "source": {                             // 条目的 canonical source
    "kind": "github-repo",                // 或 "clawhub-skill"
    "repo": "owner/repo"
  },
  "upstream": {                           // 可选；对 registry-native 来源很有用
    "kind": "github-repo",
    "repo": "owner/repo"
  },
...
- `github-repo`：使用 `source.repo = "owner/repo"`
- `clawhub-skill`：使用 `source.site`、`source.slug`、`source.url`；若是 owner 维度 URL，可额外带 `source.owner`

兼容说明：

- 现有顶层 `github` 字段短期内仍可用，但新的提交请统一使用 `source`
...
3. 新条目请使用结构化 `source` 对象；顶层 `github` 仅保留给历史兼容。
4. 提交 PR，标题格式为：`feat: add <your-skill-name>`。
5. 合并后，Dashboard 会在下一次构建时自动收录。
